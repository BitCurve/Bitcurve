(function() {
	...
	angular.module('outputValue.directives', [])

	.directive('outputValueData', [function ($location){  
		return {
			scope: {
				selectedData: '='
			}, 	// end scope
        ...
        //custom chart that takes in data 
          var custom_chart = function(data) {
            console.log("data", data);
              ...
              max_OP = d3.max(data, function(d) { return parseFloat(d.totalOutputVolumeValue, 10); }); //function for the max data and parsing it into #
              // console.log("max_OP", max_OP);
              min_OP = d3.min(data, function(d) { return parseFloat(d.totalOutputVolumeValue, 10); }); //function for the max data and parsing it into #
              // console.log("min_OP", min_OP);
            ...
            var groupLevel = function(){
              ...
              OPRange = parseFloat(max_OP - min_OP).toFixed(5);
              // console.log("OPRange", OPRange);
              lowOPRange = parseFloat(min_OP + (OPRange / 3)).toFixed(5);
              // console.log("lowOPRange", lowOPRange);
              aveOPRange = parseFloat(min_OP + ((OPRange / 3) * 2)).toFixed(5);
              // console.log("aveOPRange", aveOPRange);
            };
            groupLevel();
            ...
            data.forEach(function(d){//The forEach() method executes a provided function once per array element.
              ...
              // ***OP RANGE CONDITIONALS***
              if (d.totalOutputVolumeValue >= min_OP && d.totalOutputVolumeValue <= lowOPRange) {
                // console.log("low OP range", d.totalOutputVolumeValue);
                d.group = "low";
              }
              else if (d.totalOutputVolumeValue > lowOPRange && d.totalOutputVolumeValue <= aveOPRange) {
                // console.log("median OP range", d.totalOutputVolumeValue);
                d.group = "median";
              }
              else if (d.totalOutputVolumeValue > aveOPRange && d.totalOutputVolumeValue <= max_OP) {
                // console.log("high OP range", d.totalOutputVolumeValue);
                d.group = "high";
              }
              ...
              var node = { //refer data json
                ...
                op: parseFloat(d.totalOutputVolumeValue),
                group: d.group, 
                radius: radius_scale(parseFloat(d.totalOutputVolumeValue, 10)),
                // value: d.totalOutputVolumeValue,
          ...
          vis = d3.select("#opVis").append("svg") //this "#vis" is in index
                  .attr("width", width)
                  .attr("height", height)
                  .attr("id", "svg_vis"); 
      ...
      //GROUPING THE DATA 
       ...
       //this cannot be moved to 
        function show_details(data, i, element) {
          ...
          var content = "<span class=\"name\">OP:</span><span class=\"value\"> $" + addCommas(data.op) + "</span><br/>";
       ...
       //tooltip to hide data details till executred
        ...
        my_mod.display_year = displayOPByYear; //display year
        my_mod.toggle_view = function(view_type) { 
          if (view_type === 'outputValue') {
            displayOPByYear();
          } 
          else if (view_type === 'opYear') {
            displayOPByYear();
          } 
      ...
      $("#opVis").append("<div class='tooltip' id='"+tooltipId+"'></div>");
