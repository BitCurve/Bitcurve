(function() {
	...
	angular.module('fees.directives', [])

	.directive('feesData', [function ($location){  
		return {
			scope: {
				selectedData: '='
			}, 	// end scope
        ...
        //custom chart that takes in data 
          var custom_chart = function(data) {
            ...
            var groupLevel = function(){
              ...
              feesRange = parseFloat(max_fees - min_fees).toFixed(5);
              // console.log("feesRange", feesRange);
              lowfeesRange = parseFloat(min_fees + (feesRange / 3)).toFixed(5);
              // console.log("lowfeesRange", lowfeesRange);
              avefeesRange = parseFloat(min_fees + ((feesRange / 3) * 2)).toFixed(5);
              // console.log("avefeesRange", avefeesRange);
            };
            groupLevel();
            ...
            data.forEach(function(d){//The forEach() method executes a provided function once per array element.
              ...
              if (d.totalTransactionFees >= min_fees && d.totalTransactionFees <= lowfeesRange) {
                // console.log("low fees range", d.totalTransactionFees);
                d.group = "low";
              }
              else if (d.totalTransactionFees > lowfeesRange && d.totalTransactionFees <= avefeesRange) {
                // console.log("median fees range", d.totalTransactionFees);
                d.group = "median";
              }
              else if (d.totalTransactionFees > avefeesRange && d.totalTransactionFees <= max_fees) {
                // console.log("high fees range", d.totalTransactionFees);
                d.group = "high";
              }
      ...
      //GROUPING THE DATA 
       ...
       //tooltip to hide data details till executred
        ...
        my_mod.display_year = displayfeesByYear; //display year
        my_mod.toggle_view = function(view_type) { 
          if (view_type === 'fees') {
            displayfeesByYear();
          } 
          else if (view_type === 'feesYear') {
            displayfeesByYear();
          } 
