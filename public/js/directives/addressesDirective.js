(function() {
	...
	.directive('addressesData', [function ($location){  
        ...
        //custom chart that takes in data 
          var custom_chart = function(data) {
            console.log("data", data);
              ...
              max_address = d3.max(data, function(d) { return parseFloat(d.numberofUniqueBitcoinAddresses, 10); }); //function for the max data and parsing it into #
              // console.log("max_address", max_address);
              min_address = d3.min(data, function(d) { return parseFloat(d.numberofUniqueBitcoinAddresses, 10); }); //function for the max data and parsing it into #
              // console.log("min_address", min_address);
              radius_scale = d3.scale.pow().exponent(0.5) //pow.exponent takes in an exponent value
              .domain([0, max_address])
              .range([2, 8]);


            var groupLevel = function(){
              // console.log("the low is 0");
              addressRange = parseFloat(max_address - min_address).toFixed(5);
              // console.log("addressRange", addressRange);
              lowaddressRange = parseFloat(min_address + (addressRange / 3)).toFixed(5);
              // console.log("lowaddressRange", lowaddressRange);
              aveaddressRange = parseFloat(min_address + ((addressRange / 3) * 2)).toFixed(5);
              // console.log("aveaddressRange", aveaddressRange);
            };
            groupLevel();
            ...
            data.forEach(function(d){//The forEach() method executes a provided function once per array element.
              ...
              // ***ADDRESS RANGE CONDITIONALS***
              if (d.numberofUniqueBitcoinAddresses >= min_address && d.numberofUniqueBitcoinAddresses <= lowaddressRange) {
                // console.log("low address range", d.numberofUniqueBitcoinAddresses);
                d.group = "low";
              }
              else if (d.numberofUniqueBitcoinAddresses > lowaddressRange && d.numberofUniqueBitcoinAddresses <= aveaddressRange) {
                // console.log("median address range", d.numberofUniqueBitcoinAddresses);
                d.group = "median";
              }
              else if (d.numberofUniqueBitcoinAddresses > aveaddressRange && d.numberofUniqueBitcoinAddresses <= max_address) {
                // console.log("high address range", d.numberofUniqueBitcoinAddresses);
                d.group = "high";
              }
              ...
              var node = { //refer data json
                ...
                addresses: parseFloat(d.numberofUniqueBitcoinAddresses),
                group: d.group, 
                radius: radius_scale(parseFloat(d.numberofUniqueBitcoinAddresses, 10)),
                // value: d.numberofUniqueBitcoinAddresses,
          ...
          vis = d3.select("#addVis").append("svg") //this "#vis" is in index
                  .attr("width", width)
                  .attr("height", height)
                  .attr("id", "svg_vis"); 
      ...
      //GROUPING THE DATA 
        ...
        function displayaddressesByYear() {
       ...
       //this cannot be moved to 
        function show_details(data, i, element) {
          ...
          var content = "<span class=\"name\">Addresses:</span><span class=\"value\"> " + addCommas(data.addresses) + "</span><br/>";
       ...
       //tooltip to hide data details till executred
        ...
        my_mod.display_year = displayaddressesByYear; //display year
        my_mod.toggle_view = function(view_type) { 
          if (view_type === 'addresses') {
            displayaddressesByYear();
          } 
          else if (view_type === 'addYear') {
            displayaddressesByYear();
          } 
